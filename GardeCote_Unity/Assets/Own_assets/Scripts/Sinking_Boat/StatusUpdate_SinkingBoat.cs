...
using UnityEngine;
using System;
public class StatusUpdate_SinkingBoat : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if((drone.transform.position.x-container_ship.transform.position.x > -70) & (drone.transform.position.z-container_ship.transform.position.z < 3 & drone.transform.position.z-container_ship.transform.position.z > -3) & !drone_arrived){
        ...
        else if(analyzing & Convert.ToInt32((DateTime.UtcNow - start).TotalSeconds) > 20 & !terminated){
            PlayerPrefs.SetString("status", "Boat is sinking");
